...
**不拆分 events，而是建索引。** Event 保持原样完整存储。Entity DB 以代码实体（函数/类/方法）的 qualified name 为主键，每个 entity 下挂载指向相关 events 的引用列表。这避免了数据膨胀，同时精确到函数级的查询。
...
用户悬停/查询某个函数
    │
    ├── 1. Entity DB 查找：qualified_name → entity_id
    │      返回：signature, params, return_type, event_refs, is_external
    │
    ├── 2. Event Store 回溯：取所有关联 events 的 intent + reasoning
    │      按时间排序，重点取 created_by_event
    │
    ├── 3. 外部依赖处理（if is_external）：
    │      拉取 package docs（doc_uri）的相关段落
    │
    ├── 4. 上下文拼装 → LLM 生成解释
    │      输入：signature + intent 链 + reasoning + docs 片段
    │      输出：结构化的解释（作用、参数含义、使用场景、关联函数）
    │
    └── 5. 缓存：entity 未被新 event 修改前，解释可永久缓存
...
- **输出**：per-entity 的结构化描述（填充 Entity DB 的 description 字段）
- **可用小模型处理**，任务窄、可并行
